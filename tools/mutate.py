...
MUTATIONS = [
    ...
    ("the decoy is any other subject rather than the nearest",
     '            if overlap > best_overlap:', '            if overlap >= best_overlap:'),
    ("a subject is told apart from itself",
     '            if other_id == subject_id:\n                continue\n', ''),
    ("a test runs with nothing to tell the subject apart from",
     '        if not decoy_id:', '        if False:'),
    ("the same question can be asked again until the answer suits",
     '        if int(response.tests) > 0 and overlap <= int(response.decoy_overlap):', '        if False:'),
